...
def run_command(cmd_args: list[str], timeout: int):
    def log_error(print_msg, log_msg):
        logger.error(print_msg)
        with open(args.failed_file, "at") as fp:
            fp.write(log_msg)
    ...
    try:
        ...
        logger.info(f"Command {p.args} exited with code: {p.returncode}")
        logger.info(p.stdout)

    except subprocess.CalledProcessError as e:
        pmsg = f"Command {e.cmd} exited with code: {e.returncode}"
        lmsg = f"{pmsg}\n{e.stdout}\n\n"
        log_error(pmsg, lmsg)

    except subprocess.TimeoutExpired as e:
        pmsg = f"Command {e.cmd} timed out after {e.timeout} seconds"
        lmsg = f"{pmsg}\n{e.stdout}\n\n"
        log_error(pmsg, lmsg)

    except Exception:
        pmsg = f"Command {cmd_args} raised an exception: {traceback.format_exc()}"
        lmsg = f"{pmsg}\n\n"
        log_error(pmsg, lmsg)
...
if __name__ == "__main__":
    ...
    aclass = action2class.get(args.action, None)
